...
def fetch_data_values(
    dhis, deg_external_reference, org_unit_ids, periods, activities, package_id, path, extract
):
    """
    ...
    extract: bool
        Whether to extract the data or not.
    """
    for monthly_period in periods:
        if os.path.exists(f"{path}/{package_id}/{monthly_period}.csv") and not extract:
