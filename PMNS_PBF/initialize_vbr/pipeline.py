...
@parameter(
    "hesabu_con", type=str, help="Connection to hesabu", default="hesabu", required=True
)
@parameter(
    "program_id",
    type=str,
...
def buu_init_vbr(
    ...
    quant = prepare_quantity_data(
        done, periods, packages, contracts, hesabu_params, extract
    )
            ...
            if hesabu_params["packages"][str(id)] == "quantite" and os.path.exists(
                ...
                for p in periods:
                    if os.path.exists(f"{workspace.files_path}/packages/{id}/{p}.csv"):
                        df = pd.read_csv(
                            f"{workspace.files_path}/packages/{id}/{p}.csv"
                        )
...
@buu_init_vbr.task
def extract_data(list_dataset_id, dhis, periods, extract):
    """Extract data from the datasets."""
    os.makedirs(f"{workspace.files_path}/extracts", exist_ok=True)
    data = pd.DataFrame()
    for ds_id in [ds_id for ds_id in list_dataset_id if ds_id is not None]:
        if extract:
            print(type(periods[0]))
            period_type = dhis.api.get(f"dataSets/{ds_id}.json").json()["periodType"]
            print(period_type)
            if period_type == "Monthly" and "Q" in str(periods[0]):
                periods_collect = [
                    str(rbv_environment.quarter_to_months(p) - month)
                    for p in periods
                    for month in range(3)
                ]
            elif period_type == "Quarterly" and "Q" not in str(periods[0]):
                periods_collect = [
                    rbv_environment.month_to_quarter(int(p)) for p in periods
                ]
            else:
                periods_collect = periods
            print(periods_collect)
            orgunits = [
                ou["id"]
                for ou in dhis.api.get(f"dataSets/{ds_id}.json").json()[
                    "organisationUnits"
                ]
            ]
            ds_name = dhis.api.get(f"dataSets/{ds_id}.json").json()["name"]
            current_run.log_info(f"Extracting {ds_name}-{ds_id}.csv")
            df = dhis.data_value_sets.get(
                datasets=[ds_id],
                org_units=orgunits,
                periods=periods_collect,
            )
            df = pd.DataFrame(df)
            data = pd.concat([data, df], ignore_index=True)
            df.to_csv(f"{workspace.files_path}/extracts/{ds_name}-{ds_id}.csv")
        else:
            ds_name = dhis.api.get(f"dataSets/{ds_id}.json").json()["name"]
            try:
                df = pd.read_csv(
                    f"{workspace.files_path}/extracts/{ds_name}-{ds_id}.csv"
                )
                data = pd.concat([data, df], ignore_index=True)
            except:
                current_run.log_info(f"Could not find {ds_name}-{ds_id}.csv")
                continue
    data.rename(
        columns={"dataElement": "dx", "orgUnit": "ou", "period": "pe"}, inplace=True
    )
    data = dhis.meta.add_dx_name_column(data)
    data = dhis.meta.add_org_unit_name_column(data)
    data = rbv_environment.add_higher_levels_and_names(dhis, data)
    # data = dhis.meta.add_org_unit_parent_columns(data)
    data.to_csv(f"{workspace.files_path}/extracts/total.csv", index=False)
    return data
