...
def run_simulation(
        ...
        for subdir in path[dir]:
            ...
            for subdir2 in path[dir][subdir]:
                if not os.path.exists(
                    os.path.join(workspace.files_path, dir, subdir, subdir2)
                ):
                    os.makedirs(
                        os.path.join(workspace.files_path, dir, subdir, subdir2)
                    )
    ...
    for month in [
        int(str(m))
        for m in rbv_environment.get_date_series(str(start), str(end), frequence)
    ]:
...
def simulate(
    ...
    file_result_path = os.path.join(
        FILE_RESULTS_PATH, f"model___{model_name}-{FILE_RESULTS_NAME}"
    )

    try:
        new = False
        f = pd.read_csv(file_result_path + ".csv")
    except:
        new = True
        f = pd.DataFrame(
            columns=[
    ...
    for group in regions:
        for ou in group.members:
            ...
            categorize_quantity(
                ou, GAIN_MEDIAN_SEUIL, seuil_max_bas_risk, seuil_max_moyen_risk
            )
        ...
        detailled_list_dx.to_csv(
            f"/home/hexa/workspace/{folder}/data/listes_detaillees/-province:{group.name}-periode:{period}-detailled.csv",
            index=False,
        )
